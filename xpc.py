...
class XPlaneConnect(object):
    ...
    def setCONN(self, port):
        ...
        #Send command
        buffer = struct.pack("<4sxH".encode(), "CONN", port)
        self.sendUDP(buffer)
    ...
    def getPOSI(self, ac = 0):
        ...
        '''
        # Send request
        buffer = struct.pack("<4sxB".encode(), "GETP".encode(), ac)
        self.sendUDP(buffer)
        ...
        result = struct.unpack("<4sxBffffffffffffffff", resultBuf)
        if result[0] != b'POSI':
    ...
    def sendPOSI(self, values, ac = 0):
        ...
        # Pack message
        buffer = struct.pack("<4sxB".encode(), "POSI".encode(), ac)
        for i in range(7):
            val = -998
            if i < len(values):
                val = values[i]
            buffer += struct.pack("<f".encode(), val)
    ...
    def getCTRL(self, ac = 0):
        ...
        '''
        # Send request
        buffer = struct.pack("<4sxB".encode(), "GETC".encode(), ac)
        self.sendUDP(buffer)
    ...
    def sendCTRL(self, values, ac = 0):
        ...
        # Pack message
        buffer = struct.pack("<4sx".encode(), "CTRL".encode())
        for i in range(6):
            val = -998
            if i < len(values):
                val = values[i]
            if i == 4:
                ...
                buffer += struct.pack("b".encode(), val)
            else:
                buffer += struct.pack("<f".encode(), val)
        buffer += struct.pack("B".encode(), ac)
        if len(values) == 7:
            buffer += struct.pack("<f".encode(), values[6])
    ...
    def sendDREFs(self, drefs, values):
        ...
        buffer = struct.pack("<4sx".encode(), "DREF")
        for i in range(len(drefs)):
            ...
            if hasattr(value, "__len__"):
                ...
                buffer += struct.pack(fmt.encode(), len(dref), dref, len(value), value)
            else:
                fmt = "<B{0:d}sBf".format(len(dref))
                buffer += struct.pack(fmt.encode(), len(dref), dref, 1, value)
    ...
    def getDREFs(self, drefs):
        ...
        '''
        # Send request
        buffer = struct.pack("<4sxB".encode(), "GETD".encode(), len(drefs))
        for dref in drefs:
            fmt = "<B{0:d}s".format(len(dref))
            buffer += struct.pack(fmt.encode(), len(dref), dref)
        self.sendUDP(buffer)

        # Read and parse response
        buffer = self.readUDP()
        resultCount = struct.unpack_from("B".encode(), buffer, 5)[0]
        offset = 6
        result = []
        for i in range(resultCount):
            rowLen = struct.unpack_from("B".encode(), buffer, offset)[0]
            offset += 1
